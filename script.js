...
var upgrades = [
	{name:'Bank',price:42.32,dec:'Earn 50 DKK more when offline',priceboost:4.9},
	...
	{name:'Online Production',price:120.21,dec:'+0.10 DKK every secound',priceboost:3.2}
]
var classesType1 = {
			...
			if (userdata.inv[i] != undefined) {
				...
				if (data != false) {
					...
					iteminner.addEventListener('click', function() {
						...
						$('#inspect_price').innerHTML = (data.price * stattrak * wtp[userdata.inv[i].t]).toFixed(2) + ' DKK';
					...
					sellbtn.innerHTML = 'Sælg ' + price.toFixed(2) + ' DKK';
...
function rendCaseShop() {
	...
	for (var i = 0; i < items.length; i++) {
		...
		if (data != false && data.type == 'case') {
			...
			buybtn.innerHTML = 'Buy for ' + data.price.toFixed(2) + ' DKK';
...
function rendUpgrades() {
	...
	for (var i = 0; i < upgrades.length; i++) {var g = document.createElement('div');
		...
		btn.innerHTML = 'Buy for ' + price + ' DKK';
...
function checkRank() {
	...
	$('#rankTextRight').innerHTML = '<span>Earn</span> ' + (ranks[rank] - invv).toFixed(0) + ' DKK <span>More</span>';
	...
	$('#rankText').innerHTML = 'Value: ' + (invv + dinvv).toFixed(2) + ' DKK';
...
function update() {
	if (canUpdate) {
		$('#cash').innerHTML = userdata.money.toFixed(2) + ' DKK';
...
function rendMoveInv(TradeFunction, jj) {
	...
	$('#tradeValue').innerHTML = price.toFixed(2) + ' DKK';
...
function JPchances() {
	...
	for (var i = 0; i < jpBets.length; i++) {
		...
		joined.innerHTML = '<th><img src="https://mtsl.dk/csgo/images/' + q + '"></th><th>' + jp_chances[i].name + '</th><td>' + jp_chances[i].chance.toFixed(2) + '%</td><td>' + jpBets[i].inv.toFixed(2) + ' DKK</td>';
...
if (((new Date().getTime() - JSON.parse(localStorage['lastOnline'])) / 1000 / 60) >= 1 && lte > 0.00) {
	showPopup('offline');
	$('#msg_wb_e').innerHTML = lte.toFixed(2) + ' DKK'
